...
static constexpr const char* kCamEnable[] = { "cam_0_enable", "cam_1_enable", "cam_2_enable", "cam_3_enable", "cam_4_enable", "cam_5_enable", "cam_6_enable", "cam_7_enable"};
static constexpr const char* kCamBaseFrame[] = { "cam_0_frame_id", "cam_1_frame_id", "cam_2_frame_id",
                                                 "cam_3_frame_id", "cam_4_frame_id", "cam_5_frame_id",
...
static constexpr const char* kCamLatch[] = { "cam_0_image_latch", "cam_1__image_latch", "cam_2_image_latch",
                                             "cam_3_image_latch", "cam_4_image_latch", "cam_5_image_latch",
...
public:
  ...
  void Init()
  {
    ...
    if(Me::kNode_->encoding_ == "jpeg"){
		...
		jpeg_pub.reserve(Max_Cam);
    }else{
		this->clpe_api.Clpe_SelectFormat('1');
		kImagePubs.reserve(Max_Cam);		
    }
    ...
    // start publishing
    {
      ...
      const auto result = this->clpe_api.Clpe_StartStream(
 	    ...
 	    if(Me::kNode_->timestamp_ == "xavier"){
          ...
          static_cast<int>(this->cam_enabled_[6]), static_cast<int>(this->cam_enabled_[7]), 0);
      if (result != 0)
      {
...
*/
  ...
  {
    const auto enc = this->param<std::string>(kEncoding, "jpeg");
  ...
  {
    const auto enc = this->param<std::string>(kTimeStamp, "local");
